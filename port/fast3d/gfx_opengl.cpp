...
#ifdef PLATFORM_PORT
    { static int _ns = 0; ++_ns;
      fprintf(stderr, "[gfx] compile shader #%d id=%016llx:%08x\n",
              _ns, (unsigned long long)shader_id0, shader_id1); }
#endif
...
#ifdef PLATFORM_PORT
    /* PORT bring-up guard: a big-endian texture/tile descriptor read raw can yield an absurd
     * width/height, making glTexImage2D allocate a gigantic texture — the driver then pegs the
     * CPU (futex spin) and never returns: the "frame 4 hang". Report + skip absurd dims so the
     * frame completes. The N64 max tile is 1024x1024; allow generous slack. (M5: fix the descriptor
     * endianness so this never trips.) */
    if (width == 0 || height == 0 || width > 4096 || height > 4096 ||
        (uint64_t)width * height > (uint64_t)(2048 * 2048)) {
        static int _w = 0;
        if (_w++ < 20)
            fprintf(stderr, "[gfx] SKIP absurd texture upload %ux%u (likely big-endian descriptor)\n",
                    width, height);
        return;
    }
#endif
...
#ifdef PLATFORM_PORT
    /* Turok sets a degenerate zero-area scissor (e.g. x=0 y=240 w=320 h=0, from a G_SETSCISSOR
...
#if defined(GFX_USE_OSMESA)
#define GL_PROC_ADDR(n) OSMesaGetProcAddress(n)
#elif defined(GFX_USE_EGL)
#define GL_PROC_ADDR(n) ((void *)eglGetProcAddress(n))
#else
#define GL_PROC_ADDR(n) SDL_GL_GetProcAddress(n)
#endif

static void *gl_load_proc(const char *name) {
    void *ret = GL_PROC_ADDR(name);
    if (ret) {
        return ret;
    }
...
static void gfx_opengl_init(void) {
#if defined(GFX_USE_OSMESA) || defined(GFX_USE_EGL)
    if (!gladLoadGLLoader(gl_load_proc) || glGetString == NULL || glEnable == NULL) {
        sysFatalError("Could not load OpenGL (headless).");
    }
    // OSMesa (llvmpipe) / EGL surfaceless both hand us a compatibility desktop GL context.
    ...
    if (gl_core_profile || gl_es
        ...
        /* EGL/GBM may hand back a CORE context (no default VAO) even when we request compat, and
         * the headless path hardcodes gl_core_profile=false — without a bound VAO glDrawArrays
         * silently renders nothing (the all-black FBO). Bind one unconditionally here; harmless
         * in a compat context. */
        || true
#endif
        ) {
