...
int   g_cfg_swap_sticks  = 0;        /* New 3DS dual-analog: 0 (DEFAULT) = nub MOVES (fwd/back+strafe) + Circle Pad LOOKS (yaw+pitch); 1 = swapped (Circle Pad moves, nub looks). Options-menu toggle; turok.cfg `swap_sticks`. OG 3DS (no nub) ignores it — it stays on the classic single-stick move+turn scheme. */
