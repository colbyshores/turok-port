...
float g_cfg_stereo_strength = 3.0f;  /* 3DS stereo POP multiplier: scales BOTH shear terms together (keeps the convergence plane fixed, just more/less depth). 3.0 (DEFAULT = the options-menu "3d depth max"); 1.0 = the compiled look. Lower it in the options menu if the 3D is too strong; real-HW only. turok.cfg `stereo_strength`. */
