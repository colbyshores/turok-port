...
int   g_cfg_swap_sticks  = 0;        /* New 3DS dual-analog (matches ../perfect_dark): 0 (DEFAULT) = Circle Pad MOVES (fwd/back+strafe) + C-stick AIMS (yaw+pitch); 1 = swapped (C-stick moves, Circle Pad aims). Options-menu toggle; turok.cfg `swap_sticks`. Distinct from the right/left-handed C-button option. OG 3DS (no nub) ignores it — stays on the classic single-stick move+turn scheme. */
