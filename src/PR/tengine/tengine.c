								...
								BOOL StoreWarpReturn)
{
	...
	if (pThis->m_Warp == WARP_NOT_WARPING)
	{
		if (StoreWarpReturn)
		{
			...
			pPlayer = CEngineApp__GetPlayer(pThis);
#ifdef PLATFORM_PORT
			/* PORT: the RETURN spawn (scene.c CScene__RequestWarpPoints RETURN branch -> the player
			 * injection at ~scene.c:1701/1710) consumes m_WarpPoint with the CART warp-point endianness
			 * convention: m_vPos + m_nLevel are read HOST, but m_RotY + m_nRegion are read BIG-ENDIAN (the
			 * spawn / instance decoder ORDERBYTES them back to host). The store saved everything in HOST
			 * order, so on return m_RotY + m_nRegion byte-swapped to GARBAGE: a wrong region (no ground at
			 * the entry spot -> the player dropped OUT OF BOUNDS) + a wrong facing. Encode those two fields
			 * BIG-ENDIAN here to match the spawn. (m_vPos + m_nLevel are consumed host-side, so stay host.) */
			pThis->m_ReturnWarp.m_vPos = pPlayer->ah.ih.m_vPos;                               /* HOST */
			pThis->m_ReturnWarp.m_nLevel = pThis->m_Scene.m_nLevel;                           /* HOST */
			{ union { float f; unsigned int u; } _r; _r.f = pPlayer->m_RotY; _r.u = __builtin_bswap32(_r.u);
			  pThis->m_ReturnWarp.m_RotY = _r.f; }                                             /* host float -> BIG */
			{ WORD _rgn = (WORD) CScene__GetRegionIndex(&pThis->m_Scene, pPlayer->ah.ih.m_pCurrentRegion);
			  pThis->m_ReturnWarp.m_nRegion = __builtin_bswap16(_rgn); }                       /* host -> BIG */
#else
