...
/*-YASU*
  ...
  rsp で 40 倍の計算を楽に行うために 10 倍した値を rsp で 4 倍しているが,
  これを 5 倍にして 8 倍しても問題ないそうである. (岩田さん情報)
  ただしこのままだと頂点数 256/5 = 51 頂点までしかサポートできないのでな
  んとかする必要がある.
  
  G_TRI1 のフォーマットを変更すると何とかなりそうなので, このままでも良い
  かもしれないが, 抜本的な変更も欲しいところ.
  
  頂点バッファのサイズが変わったら真っ先に直さないといかん??????
  
  flag の値を予め 2 倍にしておくと ucode で 2 倍にする必要がない.

 7/25
  頂点キャッシュのサイズを変更し、2 の累乗にしたので(16/32bytes) 5 倍する
  必要がなくなったので素直にそのままの値を渡す.

 7/29
  flag フィールドをサポートしないことによる変更

 8/ 7
  Fast3D と F3DLP との共用のための変更
  ucode 側が flag をサポートしないため, 従来の gSP1Triangle との整合性を
  マクロでとる. コンパイラの最適化処理で flag を処理する.
  
 8/12
   No Near Clip 版のサポートにより, 頂点アドレス計算をテーブル参照によって
   行なう. 
