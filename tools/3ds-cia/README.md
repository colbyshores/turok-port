...
- `baserom.us.v12.z64` (repo root) or another region ROM — `CIA_ROM=<path>` selects the ROM. **Bundled into the CIA's RomFS** so the install is
  fully self-contained on any console (the game reads `romfs:/turok.rom`; no SD-card ROM needed).
